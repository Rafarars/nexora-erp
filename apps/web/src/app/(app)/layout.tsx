      ...
      <aside className="border-line hidden w-60 shrink-0 flex-col border-r p-4 sm:flex">
        ...
        <div className="flex-1">
          <Sidebar
            ...
            showReports={visibleReportSections(session).length > 0}
          />
        </div>
        ...
        <AccountMenu
          name={session.name}
          email={session.email}
          canAdminister={['access.users.search', 'access.roles.search', 'company.profile.search', 'company.settings.update', 'company.rates.search'].some((permission) => can(session, permission))}
        />
      </aside>
