...
export interface ReceivableBalances {
  ...
  lockCustomer(tx: TransactionClient, tenantId: string, customerId: string, today: string): Promise<CustomerExposure>;
  // Sin bloqueo, para rechazar pronto.
  exposure(db: TransactionClient, tenantId: string, customerId: string, today: string): Promise<CustomerExposure>;
