...
export async function orderDetails(
  ...
  return {
    ...
    orderDate: input.date ? PurchaseDate.of(input.date) : PurchaseDate.fromDate(now),
...
export class PurchaseOrderCreator {
  ...
  async run(request: PurchaseOrderCreatorRequest): Promise<void> {
    ...
    const details = await orderDetails(this.references, tenantId, request, now);
    ...
    PurchaseOrder.draft(id, tenantId, purchasingCode('OC', 0), details, now);

    const code = purchasingCode('OC', await this.codes.next(tenantId, 'OC'));

    await this.orders.save(PurchaseOrder.draft(id, tenantId, code, details, now));
  }
}
