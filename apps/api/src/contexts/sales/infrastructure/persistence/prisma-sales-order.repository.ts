    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (!exists) {
        ...
        const { count } = await tx.salesOrder.updateMany({
          ...
          data: { customerId: row.customerId, warehouseId: row.warehouseId, orderDate: asDate(orderDate), notes: row.notes, updatedAt: row.updatedAt },
        });

        if (count === 0) throw new SalesOrderNotEditableError(row.id, exists.status);
