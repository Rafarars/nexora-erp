...
// Capa anticorrupcion para el kardex: lee los codigos de los ajustes y, sin importar nada
// de compras, los de sus entradas de mercancia.
@Injectable()
export class PrismaMovementDocuments implements MovementDocuments {
  ...
  async codesOf(tenantId: TenantId, documents: DocumentRef[]): Promise<Map<string, string>> {
    ...
    const [adjustments, receipts] = await Promise.all([
    ...
    return new Map([...adjustments, ...receipts].map((row) => [row.id, row.code]));
  }
}
