...
describe('ItemSearcher', () => {
  it('returns each item with the names of what it references and the base unit first', async () => {
    ...
    const { items } = await new ItemSearcher(scenario.items, scenario.categories, scenario.taxes, scenario.units).run({
      tenantId: TENANT_A,
    });
  ...
  it('never lists an item of another tenant', async () => {
    const scenario = aCatalogScenario({ items: [anItem({ id: ITEM_B, tenantId: TENANT_B })] });
