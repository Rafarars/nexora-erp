...
// inventario, compras y ventas sin importar esos contextos, como la existencia de las bodegas.
@Injectable()
export class PrismaItemPosting implements ItemPosting {
  ...
  async post(tenantId: TenantId, itemId: ItemId, work: (item: Item, commitments: ItemCommitments) => void): Promise<void> {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      const item = itemFromRow(await tx.item.findFirstOrThrow({ where: { tenantId: tenant, id }, include: { units: true } }));
      ...
      work(item, {
        ...
        openDocumentUnits: openUnits.map((row) => MeasurementUnitId.of(row.unit_id)),
      });
