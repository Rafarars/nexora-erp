...
describe('Item', () => {
  ...
  it('knows what it uses, so the catalog can refuse to deactivate it', () => {
    ...
    expect(item.usesCategory(CategoryId.of(CATEGORY_A))).toBe(true);
    expect(item.usesCategory(CategoryId.of(CATEGORY_B))).toBe(false);
    expect(item.usesTax(TaxId.of(TAX_A))).toBe(true);
    expect(item.usesUnit(MeasurementUnitId.of(UNIT_PIECE))).toBe(true);
    expect(item.usesUnit(MeasurementUnitId.of(UNIT_BOX))).toBe(false);
  });

  it('without a category, uses no category at all', () => {
    expect(anItem({ categoryId: null }).usesCategory(CategoryId.of(CATEGORY_A))).toBe(false);
  });
    ...
    item.update(
      {
        ...
        description: '  Botella  ',
        type: 'inventoried',
        categoryId: CategoryId.of(CATEGORY_B),
        taxId: null,
