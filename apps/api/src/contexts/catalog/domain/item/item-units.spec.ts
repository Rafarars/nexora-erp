...
import { InvalidConversionFactorError, InvalidItemUnitsError } from '../errors/invalid-values.errors.js';
import { UNIT_BOX, UNIT_KILO, UNIT_PIECE } from '../testing/catalog.mother.js';
...
describe('ItemUnits', () => {
  it('accepts a base unit and a box of 24', () => {
    ...
    expect(units.uses(MeasurementUnitId.of(UNIT_BOX))).toBe(true);
    expect(units.uses(MeasurementUnitId.of(UNIT_KILO))).toBe(false);
  });
